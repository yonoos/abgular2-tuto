...
.controller("WebmailCtrl", function($rootScope, $scope, $location, $filter, mailService) {
	...
	socket.on("newmail",function(idDossier){
		console.log("new email received in box "+idDossier);
		if($scope.dossierCourant && $scope.dossierCourant.value == idDossier){
			$scope.dossierCourant = mailService.getDossier(idDossier);
			console.log("reload folder : "+idDossier);
		} else {
			for (var i = 0; i < $scope.dossiers.length; i++) {
				var dossier = $scope.dossiers[i]; 
				if(dossier.value == idDossier){
					dossier.needsUpdate = true;
					console.log("set dossier new : "+idDossier);
				}
			}
		}
		$scope.$apply();
